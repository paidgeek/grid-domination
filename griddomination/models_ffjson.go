...
func (mj *Cell) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	...
	buf.WriteString(`,"claim_duration":`)
	fflib.FormatBits2(buf, uint64(mj.ClaimDuration), 10, mj.ClaimDuration < 0)
	if mj.IsOwned {
...
var ffj_key_Cell_ClaimDuration = []byte("claim_duration")
	...
	for {
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				...
				case 'c':
					...
					} else if bytes.Equal(ffj_key_Cell_ClaimDuration, kn) {
						currentKey = ffj_t_Cell_ClaimDuration
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.AsciiEqualFold(ffj_key_Cell_ClaimDuration, kn) {
					currentKey = ffj_t_Cell_ClaimDuration
					state = fflib.FFParse_want_colon
					goto mainparse
				}
		...
		case fflib.FFParse_want_value:
			...
			if tok == fflib.FFTok_left_brace || tok == fflib.FFTok_left_bracket || tok == fflib.FFTok_integer || tok == fflib.FFTok_double || tok == fflib.FFTok_string || tok == fflib.FFTok_bool || tok == fflib.FFTok_null {
				...
				case ffj_t_Cell_ClaimDuration:
					goto handle_ClaimDuration
...
handle_ClaimDuration:

	/* handler: uj.ClaimDuration type=time.Duration kind=int64 quoted=false*/
...

	{
		...
		if tok == fflib.FFTok_null {
			...
			uj.ClaimDuration = time.Duration(tval)
...

	{
		...
		if tok == fflib.FFTok_null {
			...
			for {
				...
				tok = fs.Scan()
				/* handler: tmp_uj__Cells type=*griddomination.Cell kind=ptr quoted=false*/

				{
					...
					if tmp_uj__Cells == nil {
						tmp_uj__Cells = new(Cell)
					}
